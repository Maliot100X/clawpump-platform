...
export async function GET() {
  try { const r = await mcp("list_agents"); return NextResponse.json({ agents: JSON.parse(r).agents || [] }); } catch { return NextResponse.json({ agents: [] }); }
}
export async function POST(req: NextRequest) {
  try { const b = await req.json(); const r = await mcp("create_agent", { name: b.name }); return NextResponse.json({ agent: JSON.parse(r).agent || JSON.parse(r) }); } catch { return NextResponse.json({ error: "Failed" }, { status: 500 }); }
}
